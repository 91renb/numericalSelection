...
- (void)setDatas:(id)datas
{
    ...
    self.detailTextLabel.text = [[type stringByAppendingString:@"  "] stringByAppendingFormat:@"%ld注%ld元",[datas[@"bettingNum"] integerValue],[datas[@"bettingNum"] integerValue] * 2];
}
@end
...
//                 [weakSelf requestGotoBetting];
                 [SVProgressHUD showInfoWithStatus:@"投注成功!!!\n注意：该彩票只是模拟彩票投注，已为您加入收藏！"];
             }
         }class:@"LoginVC"];
        return;
    }
