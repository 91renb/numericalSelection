...
{
    NSString *money = [NSString stringWithFormat:@"%ld",2 * bettingNum * FBTool.multiple];

    NSString *text = [NSString stringWithFormat:@"%ld注 %ld倍 共 %@ 元",(long)bettingNum,(long)FBTool.multiple, money];
...
//                 [weakSelf requestGotoBetting];
                 [SVProgressHUD showSuccessWithStatus:@"投注成功!!\n注意，本彩票只是模拟投注，已为你加入收藏成功！"];
